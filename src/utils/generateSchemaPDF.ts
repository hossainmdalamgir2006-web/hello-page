...
export function generateSchemaPDF(storeName?: string): void {
  const doc = new jsPDF();
  ...
  const drawHeader = () => {
    ...
    doc.text(storeName || "YOUR STORE", margin, 15);
  ...
  const drawFooter = () => {
    ...
    doc.text(`© ${storeName || "YOUR STORE"} - Database Documentation`, margin, pageHeight - 10);
  };
