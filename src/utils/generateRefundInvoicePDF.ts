...
async function getStoreConfig() {
  try {
    const [settingsRes, siteContentRes, templateRes] = await Promise.all([
      supabase.from("store_settings").select("key, setting_value")
        .in("key", ["STORE_EMAIL", "STORE_PHONE", "STORE_ADDRESS"]),
      supabase.from("site_content_overrides" as any).select("content")
        .eq("page_slug", "header").eq("section_key", "main_content").maybeSingle(),
    ...
    const hdr = (siteContentRes.data as any)?.content as any;
    const tplCfg = templateRes.data?.config as any;

    return {
      store_name: hdr?.store_name || tplCfg?.store_name || "Your Store",
      store_logo_url: hdr?.store_logo || tplCfg?.store_logo_url || "",
      store_address: tplCfg?.store_address || map["STORE_ADDRESS"] || "",
      store_email: tplCfg?.store_email || map["STORE_EMAIL"] || "",
      store_phone: tplCfg?.store_phone || map["STORE_PHONE"] || "",
    };
  } catch {
