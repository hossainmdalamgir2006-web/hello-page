...
export function CartDrawer() {
  const {
    items, isOpen, setIsOpen, removeItem, updateQuantity, itemCount,
    selectedKeys, toggleSelected, selectedSubtotal, selectedCount,
  } = useCart();
  const { t } = useLanguage();
          ...
          <>
            ...
            {selectedCount < itemCount && (
              <p className="text-xs text-muted-foreground px-1 -mb-2">
                Tick items to include in checkout
              </p>
            )}
            ...
            <div className="flex-1 overflow-y-auto py-4 space-y-3">
              {items.map((item) => {
                ...
                return (
                  <div
                    key={k}
                    className={`flex gap-3 p-3 rounded-lg transition-colors ${isSelected ? 'bg-store-muted/50' : 'bg-muted/20 border border-dashed border-border'}`}
                  >
                    <div className="flex items-start pt-1">
                      <Checkbox
                        checked={isSelected}
                        onCheckedChange={() => toggleSelected(k)}
                        className="h-5 w-5"
                      />
                    </div>
            ...
            <div className="border-t pt-4 space-y-4">
              <div className="flex justify-between text-sm">
                <span className="text-muted-foreground">
                  {t('store.subtotal')} <span className="text-xs">({selectedCount} of {itemCount} selected)</span>
                </span>
