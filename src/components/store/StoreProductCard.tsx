      ...
      <Card className="overflow-hidden">
        <div className="flex">
          <Link to={productUrl} className="relative w-32 sm:w-48 flex-shrink-0">
            ...
            {isNew && !discount && (
              <Badge className="absolute top-2 left-2 bg-store-primary text-store-primary-foreground">
                New
              </Badge>
            )}
          </Link>
          <CardContent className="flex-1 p-4 flex flex-col justify-between">
            <div>
              <div className="flex items-center gap-2 mb-1">
                ...
                {isBundle && <Badge variant="outline" className="text-[10px] px-1.5 py-0"><Package className="h-2.5 w-2.5 mr-0.5" />Bundle</Badge>}
                {isVariable && <Badge variant="outline" className="text-[10px] px-1.5 py-0"><Layers className="h-2.5 w-2.5 mr-0.5" />Variable</Badge>}
              </div>
              <Link to={productUrl}>
              ...
              <div className="flex items-center gap-2 mt-2">
                <span className="font-bold text-foreground">
                  {showFromPrice ? 'From ' : ''}৳{product.price.toLocaleString()}
                </span>
  ...
  return (
    ...
    <Card className="group overflow-hidden border-0 shadow-md hover:shadow-xl transition-all duration-300">
        ...
        <div className="relative aspect-[3/4] overflow-hidden bg-muted">
          ...
          <div className="absolute top-3 right-3 flex flex-col gap-1.5 opacity-0 group-hover:opacity-100 transition-opacity">
            ...
            <Button
              variant="secondary"
              size="icon"
              className="rounded-full bg-store-card/90 backdrop-blur-sm"
              onClick={(e) => { e.preventDefault(); e.stopPropagation(); setQuickViewOpen(true); }}
            >
              <Eye className="h-4 w-4" />
            </Button>
      ...
      <CardContent className="p-4">
        <div className="flex items-center gap-2 mb-1">
          <p className="text-xs text-muted-foreground">{product.category}</p>
          {isVariable && <Badge variant="outline" className="text-[9px] px-1 py-0">Variable</Badge>}
        </div>
        <Link to={productUrl}>
