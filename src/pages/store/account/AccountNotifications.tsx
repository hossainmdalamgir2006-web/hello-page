...
import { Bell, BellOff, CheckCheck, Filter, Trash2, Package, MessageCircle, Settings2, Tag, AlertCircle } from "lucide-react";
          ...
          <TabsList className="bg-muted/50 h-9">
            <TabsTrigger value="all" className="text-xs gap-1.5">
              ...
              {notifications.length > 0 && (
                <Badge variant="secondary" className="h-5 min-w-5 px-1 text-[10px]">{notifications.length}</Badge>
              )}
            </TabsTrigger>
            <TabsTrigger value="unread" className="text-xs gap-1.5">
              Unread
              {unreadCount > 0 && (
                <Badge className="h-5 min-w-5 px-1 text-[10px] bg-primary text-primary-foreground">{unreadCount}</Badge>
              )}
            </TabsTrigger>
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2">
                      <p className={cn("text-sm leading-tight", !n.is_read ? "font-semibold text-foreground" : "font-medium text-foreground/80")}>
                        {n.title}
                      </p>
                      {!n.is_read && (
                        <span className="h-2 w-2 rounded-full bg-primary shrink-0" />
                      )}
                      <Badge variant="outline" className={cn("ml-auto text-[10px] h-5 shrink-0 hidden sm:inline-flex", typeInfo.color)}>
                        {typeInfo.label}
                      </Badge>
                    </div>
                    {n.message && (
