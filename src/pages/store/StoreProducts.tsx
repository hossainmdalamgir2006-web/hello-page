...
function FilterPanel({
  ...
  return (
    <div className="space-y-6">
      <div>
        <h4 className="font-medium mb-3 text-sm uppercase tracking-wide text-muted-foreground">Categories</h4>
      ...
      <div>
        <h4 className="font-medium mb-3 text-sm uppercase tracking-wide text-muted-foreground">Price Range</h4>
        <Slider
          value={priceRange}
      ...
      <div>
        <h4 className="font-medium mb-3 text-sm uppercase tracking-wide text-muted-foreground">Quick Filters</h4>
        <div className="space-y-2">
      ...
      <div className="flex gap-3">
        ...
        {onApply && (
          <Button className="flex-1 bg-store-primary hover:bg-store-primary/90" onClick={onApply}>
            Apply
          </Button>
        )}
      </div>
...
export default function StoreProducts() {
  ...
  const clearFilters = () => {
    setSearch(""); setSelectedCategories([]); setPriceRange([0, 10000]);
    setShowSale(false); setShowNew(false); setSortBy("newest");
  };

  const activeFilterCount = [search, selectedCategories.length > 0, priceRange[0] > 0 || priceRange[1] < 10000, showSale, showNew].filter(Boolean).length;
  ...
  const filterPanelProps = {
    dbCategories, selectedCategories, toggleCategory, priceRange, setPriceRange,
